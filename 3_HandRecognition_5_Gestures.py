...
# Load model.
rf:RandomForestClassifier = load(f"{currentDirectory}/RandomForest_2.joblib")
knnModel: KNeighborsClassifier = load(f"{currentDirectory}/KnnModel_2.joblib")
...
try:
    labels = ['Paper', 'Rock','V', 'Thumbs Up', 'One', 'NO HAND']
    rfPredicted = 5
    knnPredicted = 5
    while cap.isOpened():
        ret, frame = cap.read()
